...
`reporter` command is able to send a single report for multiple test runs. For
now, only runs with same patch series and base are supported. This is
especially useful if the same patch series are being tested on multiple
architectures. To run reporter in multireport mode, add `--states` option
with a list of skt state files created by `run` command you wish to report.
Using the previous example of mail report command and state files `./state_x86`
and `./state_s390x`, the command will be:

    skt --rc skt-rc --state --workdir skt-workdir -vv \
        report --reporter mail \
        '{"mailfrom": "'$USER'@localhost", "mailto": "'$USER'@localhost, root@localhost"}' \
	--states ./state_x86 ./state_s390x
