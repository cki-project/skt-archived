    ...
    for thing_to_merge in cfg.get('merge_queue', []):
        try:
            if thing_to_merge[0] == 'merge_ref':
                if not previous_merge[0] == 'merge_ref':
                ...
                if retcode:
                    if retcode != SKT_ERROR:
                        report_results(merge_result_path,
                                       'false',
                                       merge_report_path,
                                       report_string)
                ...
                report_string += '\n  - %s\n    into commit %s' % (mb[0],
                                                                   bhead[:12])
                previous_merge = thing_to_merge
                idx[0] += 1

            elif thing_to_merge[0] == 'patch' or thing_to_merge[0] == 'pw':
                if previous_merge[0] == 'merge_ref':
